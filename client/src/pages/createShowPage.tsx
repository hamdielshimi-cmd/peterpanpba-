...
const GAS_URL = 'https://script.google.com/macros/s/AKfycbw1U7M2Yo0WNrVawnO6eRzvZuvJOfKIxu5Jew9Jslq_iBbhzOs-t_A7cIAMmv2t-T-k/exec';
...
/**
 * Factory function to create a show-specific page component with a hardcoded show number.
 * This avoids code duplication across Show1.tsx through Show5.tsx
 */
export function createShowPage(showNumber: number) {
      ...
      <div style={{ minHeight: '100vh', backgroundColor: bg, color: '#e8e8e8', padding: '2rem 1rem', fontFamily: 'system-ui,sans-serif' }}>
        <div style={{ maxWidth: 520, margin: '0 auto' }}>
          ...
          <div style={{ ...card, padding: '1.5rem', marginBottom: '1rem' }}>
            ...
            {/* Step 1: Pay */}
            <div style={{ marginBottom: '0.75rem', padding: '0.75rem 1rem', background: '#2a1a2a', borderRadius: 8, fontSize: '0.85rem', color: '#ccc' }}>
              <strong style={{ color: gold }}>1. Pay via InstaPay</strong>
              <div style={{ marginTop: '0.5rem', fontSize: '0.75rem' }}>Send <strong>{totalPrice} EGP</strong> to the link below, then paste your receipt.</div>
            </div>
            <a href={INSTAPAY_LINK} target="_blank" rel="noopener noreferrer" style={{ display: 'block', width: '100%', padding: '0.8rem', background: gold, color: '#140814', textAlign: 'center', borderRadius: 8, fontWeight: 700, textDecoration: 'none', marginBottom: '1rem', border: 'none', cursor: 'pointer' }}>
              💳 Open InstaPay
            </a>
            ...
            <Button onClick={() => setPhase(1)} style={{ width: '100%', marginTop: '1rem', background: 'transparent', border: '1px solid #4a3a4a', color: dim, padding: '0.8rem', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }}>
              ← Back
            </Button>
          </div>
        </div>
