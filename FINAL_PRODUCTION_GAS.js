...
// - All columns locked except Status for protection
...
function setupProductionSheets() {
  ...
  for (const name of sheetNames) {
    ...
    // Protect sheet - allow only Status column (I) to be edited
    const protection = sheet.protect().setDescription('Protected - only Status column editable');
    protection.removeEditors(protection.getEditors());
    protection.addEditor(Session.getEffectiveUser());
    
    // Unprotect Status column (column I = 9)
    const statusRange = sheet.getRange('I:I');
    protection.unprotectRange(statusRange);
    
    // Add data validation to Status column
    const lastRow = sheet.getMaxRows();
    const statusValidation = SpreadsheetApp.newDataValidation()
      .requireValueInList(['Pending', 'Confirmed'], true)
      .setAllowInvalid(false)
      .setHelpText('Select Pending (extends 15min) or Confirmed (locks seats)')
      .build();
    sheet.getRange(`I2:I${lastRow}`).setDataValidation(statusValidation);
  }
  
  Logger.log('✅ Production sheets initialized with protection');
}
